...
function bonusUpperCalc(total, num) { // to calculate upper score
...
const lineFunctions = {
  matching(lFObjectInput) {
    ...
    const boostNeed = (bonusUpper >= 63 /*&& statistics.result.likelyValue / statistics.result.maxValue >= 0.7 */) ? 0 : 1
    conf.x.focusPrio = Math.round(statistics.result.average / statistics.result.maxValue * (1 + 2 * boostNeed) * 1000) / 1000
    conf.x.pickPrio = (conf.x.result * (1 + 1 * boostNeed)) / conf.x.potentialMax
    return conf.x;
  },
  pasch(lFObjectInput) {
    const lFObject = JSON.parse(JSON.stringify(lFObjectInput))
    ...
    const potentialArray = Array(rollsAsArray.length + 1).fill(0).map((x, i) => i >= sheetLayout[currentRow].sums ? arraySum(conf.rollsArray) : 0)
    const statistics = get_stats({
      rollsLeft: rollsLeft || 0,
      minLength: conf.x.dicesMatch.length || 0,
      possibleResults: potentialArray
  ...
  fullHouse(lFObjectInput) {
    ...
    const potentialArray = Array(rollsAsArray.length + 1).fill(0).map((x, i) => i >= needed ? fhValue : 0)
    const statistics = get_stats({
      rollsLeft: rollsLeft || 0,
      minLength: conf.x.dicesMatch.length || 0,
      possibleResults: potentialArray
  ...
  kleineStraße(lFObjectInput) {
    ...
    const potentialArray = Array(rollsAsArray.length + 1).fill(0).map((x, i) => i >= needed ? ksValue : 0)
    const statistics = get_stats({
      rollsLeft: rollsLeft || 0,
      minLength: conf.x.dicesMatch.length || 0,
      possibleResults: potentialArray
  ...
  großeStraße(lFObjectInput) {
    ...
    const potentialArray = Array(rollsAsArray.length + 1).fill(0).map((x, i) => i >= needed ? gsValue : 0)
    const statistics = get_stats({
      rollsLeft: rollsLeft || 0,
      minLength: conf.x.dicesMatch.length || 0,
      possibleResults: potentialArray
  ...
  Kniffel(lFObjectInput) {
    ...
    const potentialArray = Array(rollsAsArray.length + 1).fill(0).map((x, i) => i >= needed ? knfValue : 0)
    const statistics = get_stats({
      rollsLeft: rollsLeft || 0,
      minLength: conf.x.dicesMatch.length || 0,
      possibleResults: potentialArray
  ...
  bonus_oben(lFObjectInput) {
    ...
    // const rollsLeft = lFObject.rollsLeft
    const conf = getConf(rollsAsArray)
    conf.x.result = sheetDataColumnX[8].value >= 63 ? 35 : 0;
    return conf.x;
  }
};
